...
// Enhanced resetAnimations function
function resetAnimations() {
    ...
    // ALWAYS remove the 'highlight' class from elements (e.g., list items, text spans)
    // This resets all background highlights created by highlightListItem or highlightCode that use .highlight class
    document.querySelectorAll('.highlight').forEach(el => el.classList.remove('highlight'));

    // Reset the state of the *annotation* elements (tooltips and info boxes) for the CURRENT slide
    const activeSlide = getActiveSlide();
    if (activeSlide) {
        if (annotationsVisible) {
            // If annotations should be visible, reset them to their initial hidden state (ready to be animated in)
            annotationToggleElementsSelectors.forEach(selector => {
                activeSlide.querySelectorAll(selector).forEach(el => {
                    if (el.classList.contains('code-annotation')) {
                        el.classList.remove('show'); // Ensure tooltips are hidden initially
                    }
                    el.style.opacity = '0'; // Ensure info boxes are hidden initially
                    el.style.pointerEvents = 'none'; // Prevent interaction initially
                    el.style.display = 'none'; // Ensure elements are not visible by default
                    el.style.visibility = 'hidden'; // Ensure elements are hidden
                });
            });
        } else {
            // If annotations should be hidden globally, ensure they remain hidden (override initial state)
            // Call hideAllAnnotations on the active slide's elements
            hideAllAnnotations(); 
        }
    }

    // Reset background highlights (specific element styles, not classes)
    const bgToReset = ['#bracket-method', '#at-method']; // These are specific elements with dynamic background
...
function showElement(elementId) {
    ...
    if (element && annotationsVisible) {
        ...
        element.style.display = 'block'; // Ensure it's block for immediate visibility
